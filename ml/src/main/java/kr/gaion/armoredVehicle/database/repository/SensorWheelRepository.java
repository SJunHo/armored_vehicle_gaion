...
public interface SensorWheelRepository extends JpaRepository<SensorWheel, Long> {
    ...
    Page<SensorWheelLeftInterface> getLeftWheelAiLWPredictedData(String carId, String modelName, Date fromDate, Date toDate, Pageable pageable);
    ...
    @Query(value = "Select W.IDX, W.SDAID, W.AI_RW, W.AI_RW_ALGO, W.AI_RW_MODEL, W.AI_RW_DATE, " +
            ...
            " INNER JOIN `ENGDATA` ON W.`DATE` = `ENGDATA`.`DATE` AND W.SDAID = `ENGDATA`.SDAID ", nativeQuery = true)
    Page<SensorWheelRightInterface> getRightWheelAiRWPredictedData(String carId, String modelName, Date fromDate, Date toDate, Pageable pageable);
