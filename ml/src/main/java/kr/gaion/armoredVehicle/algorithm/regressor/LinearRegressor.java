...
public class LinearRegressor extends MLAlgorithm<BaseAlgorithmTrainInput , BaseAlgorithmPredictInput> {
    public LinearRegressor(ElasticsearchSparkService elasticsearchSparkService, DatabaseSparkService databaseSparkService, Utilities utilities,
                           StorageConfig storageConfig, ModelUtilService modelUtil, FSChiSqSelector chiSqSelector, AlgorithmConfig algorithmConfig,
                           DataConfig dataConfig, SparkSession sparkSession, ModelService modelService) {
    ...
    public RegressionResponse train(BaseAlgorithmTrainInput config) throws Exception {
        ...
        // Select example rows to display. - prediction is scaled.
        System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@ Predictions for validation ... @@@@@@@@@@@@@@@@@@@@@@@@@");
        predictions.select("prediction", "label", "features", "scaledFeatures").show(20, false);
        ...
        // R2
        response.setR2(Double.isNaN(trainingSummary.r2()) ? 0 : trainingSummary.r2());
    ...
    public RegressionResponse predict(BaseAlgorithmPredictInput input) throws Exception {
        System.out.println("============================ START Linear Regression predicting unlabeled data ============================");
        ...
        // 3. predict
        model.setFeaturesCol("scaledFeatures");
        Dataset<Row> predictedResult = model.transform(data);

        // 4. response
        var response = new RegressionResponse(ResponseType.OBJECT_DATA);

        // dataset's each rows convert to line data for response
        var lineData = doPredictRegressionData(predictedResult);
    ...
    private static JavaRDD<String> doPredictRegressionData(Dataset<Row> predictedResult) {
        Dataset<Row> tempDataset = predictedResult;
        Dataset<Row> resultDataset = tempDataset.drop("features").drop("scaledFeatures");
    ...
    private static JavaRDD<String> doPredictRegressionData_old(Dataset<Row> data, LinearRegressionModel model, List<String> fieldsForPredict) {
